...
class HierarchicalIndex:
    ...
    def __init__(self, indexer: Indexer) -> None:
        ...
        self._key_to_group_id: dict[tuple[int, IndexKey], int] = {}
        self._groups: dict[int, tuple[Row, tuple[tuple[int, IndexKey], ...]]] = {}
        self._next_group_id = 0

    def add(self, row: Row) -> None:
        """Add a row with automatically generated hierarchical keys."""
        keys = tuple(enumerate(self._indexer(row)))
    ...
    def pop(self, row: Row) -> Row | None:
        """Pop first available match for the given row, trying keys by priority."""
        for priority, key in enumerate(self._indexer(row)):
            if matched_row := self._pop_by_key(priority, key):
                return matched_row
        return None

    def _pop_by_key(self, priority: int, key: IndexKey) -> Row | None:
        """Pop value by specific priority and key, removing from all keys."""
        lookup_key = (priority, key)
        group_id = self._key_to_group_id.get(lookup_key)
        if group_id is None:
            return None
