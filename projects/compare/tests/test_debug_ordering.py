...
def test_ordering_impact():
    """Test how sort order affects RowGUID matching."""
    print("=== Test 1: alpha before beta ===")
    old_conn = connect(":memory:")
    old_conn.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, RowGUID TEXT, name TEXT)")
    old_conn.execute("INSERT INTO test VALUES (1, 'guid-alpha', 'Item A')")
    old_conn.commit()

    new_conn = connect(":memory:")
    new_conn.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, RowGUID TEXT, name TEXT)")
    new_conn.execute("INSERT INTO test VALUES (10, 'guid-alpha', 'Item A Modified')")
    new_conn.commit()

    changes = list(next(iter(compare_databases(old_conn, new_conn)))["rows"]["changes"])
    ...
    old_conn = connect(":memory:")
    old_conn.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, RowGUID TEXT, name TEXT)")
    old_conn.executemany("INSERT INTO test VALUES (?, ?, ?)", [
        (1, 'guid-alpha', 'Item A'),
        (2, 'guid-beta', 'Item B')
    ])
    old_conn.commit()
    ...
    new_conn = connect(":memory:")
    new_conn.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, RowGUID TEXT, name TEXT)")
    new_conn.executemany("INSERT INTO test VALUES (?, ?, ?)", [
    ...
    print(f"Two row test: Modified={len(modified)}, Added={len(added)}, Removed={len(removed)}")
