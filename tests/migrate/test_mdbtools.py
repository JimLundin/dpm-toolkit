...
def test_native_types_maps_replication_id_to_char32(recorded: Recorder) -> None:
    """Replication IDs become CHAR(32), not the sqlite backend's INTEGER."""
    recorded(ACCESS_DDL)

    row_guid = _native_types(UNUSED)["Release", "RowGUID"]

    assert isinstance(row_guid, CHAR)
    assert row_guid.length == 32
...
def test_coerce_repairs_guid_missing_its_fourth_hyphen() -> None:
    """The 8-4-4-16 GUID mdbtools emits normalises to bare lowercase hex."""
    column = _column("RowGUID", CHAR(32))

    coerced = _coerce(column, "{083F7C98-D0CF-4D87-A2E19DC464712242}", [])

    assert coerced == "083f7c98d0cf4d87a2e19dc464712242"
...
def test_repair_guid_returns_none_for_empty() -> None:
    """A value carrying no hex digits is a NULL GUID, not an empty string."""
